...
const DoctorAppointments = () => {
  ...
  const handleCancel = async (appointmentId) => {
    try {
      ...
      if (response.success) {
        alert(response.message); // Show success message with refund status
        ...
        <div className="bg-white border rounded-lg shadow-sm overflow-hidden">
          ...
          <div className="max-h-[70vh] overflow-y-auto">
            {appointments.map((item, index) => (
              <div
                key={index}
                className="flex flex-wrap justify-between max-sm:gap-5 max-sm:text-base sm:grid grid-cols-[0.5fr_2fr_1fr_1fr_3fr_1fr_1.5fr] gap-1 items-center text-gray-600 py-4 px-6 border-b hover:bg-gray-50 transition-colors"
              >
                ...
                <div>
                  <span
                    className={`text-xs px-3 py-1 rounded-full ${
                      item.payment ? "bg-blue-100 text-blue-600" : "bg-green-100 text-green-600"
                    }`}
                  >
                ...
                <p className="font-medium text-gray-800">
                  {currency}
                  {item.discountedAmount ? item.discountedAmount : item.originalAmount}
                </p>
