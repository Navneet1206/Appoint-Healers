...
import { Home, Users, Info, Phone, User, LogIn, LogOut, ChevronDown, Menu, X } from 'lucide-react';
...
const Navbar = () => {
  ...
  const navItems = [
    ...
    { path: '/professional', label: 'BOOK NOW', icon: <Users size={20} /> },
  ...
  return (
    <>
      ...
      <div className={`fixed top-4 left-1/2 transform -translate-x-1/2 transition-all duration-300 
      ...
      rounded-full py-3 px-6 flex items-center justify-between border border-[#FAD1E2] z-50`}>
        <h1
          onClick={() => navigate('/')}
          className="cursor-pointer text-xl xl:text-2xl font-bold text-[#9E1B5B] transition-all duration-300"
        >
          SAVAYS HEALS
        </h1>
        ...
        <ul className='hidden xl:flex items-center gap-4 lg:gap-6 font-medium text-[#9E1B5B]'>
          {navItems.map((item) => (
            <NavLink 
              ...
              to={item.path}
              className={({ isActive }) => 
                `px-3 py-1 rounded-full transition-all duration-200 ${isActive ? 'bg-[#FAD1E2] bg-opacity-50' : 'hover:bg-[#FAD1E2] hover:bg-opacity-30'}`
              }
            >
        ...
        <div className='hidden xl:flex items-center gap-4'>
          {token && userData ? (
            ...
            >
              ...
              {dropdownVisible && (
                <div
                  className='absolute right-0 mt-3 min-w-[180px] bg-white border border-gray-200 rounded-lg shadow-lg z-50 p-2 flex flex-col gap-1 text-gray-600'
                >
      ...
      <div className={`xl:hidden fixed inset-0 bg-black bg-opacity-50 z-50 transition-opacity duration-300 ${showMenu ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
        <div 
          className={`fixed top-0 right-0 bottom-0 bg-white shadow-lg w-3/4 max-w-sm z-50 transition-transform duration-300 ease-in-out rounded-l-xl ${showMenu ? 'translate-x-0' : 'translate-x-full'}`}
        >
            ...
            <ul className='flex flex-col gap-4 font-medium text-[#9E1B5B]'>
              {navItems.map((item) => (
                <NavLink 
                  ...
                  to={item.path}
                  className={({ isActive }) => 
                    `flex items-center gap-3 px-4 py-3 rounded-lg transition-all duration-200 ${isActive ? 'bg-[#FAD1E2] bg-opacity-30' : 'hover:bg-[#FAD1E2] hover:bg-opacity-10'}`
                  }
                >
                  {item.icon}
                  <span>{item.label}</span>
              ...
              {token && userData ? (
                <div className='flex flex-col gap-3'>
                  <button 
                    ...
                    className='flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-[#FAD1E2] hover:bg-opacity-10 w-full text-left text-[#9E1B5B]'
                  >
                  ...
                  <button 
                    ...
                    className='flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-[#FAD1E2] hover:bg-opacity-10 w-full text-left text-[#9E1B5B]'
                  >
                    <Users size={20} /> My Appointments
                  </button>
                  <button 
                    onClick={() => { logout(); setShowMenu(false); }}
                    className='flex items-center gap-3 px-4 py-3 bg-[#E64794] bg-opacity-10 rounded-lg hover:bg-opacity-20 w-full text-left text-[#E64794]'
                  >
                ...
                <button
                  ...
                  className='w-full bg-gradient-to-r from-[#E64794] to-[#9E1B5B] text-white px-4 py-3 rounded-xl font-medium transition-all duration-200 flex items-center justify-center gap-2'
                >
